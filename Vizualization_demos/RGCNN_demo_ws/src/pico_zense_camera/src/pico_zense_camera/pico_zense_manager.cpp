...
PicoSenseManager::PicoSenseManager(int32_t device_index, const string &camera_name, uint16_t depth_threshold,
    ...
    ROS_INFO("Set capture mode to DepthAndRGB_30 for device %d mode %d", this->device_index_,this->data_mode_);
...
void PicoSenseManager::run() {
    ...
    sensor_msgs::CameraInfoPtr ir_ci(new sensor_msgs::CameraInfo(depth_info_->getCameraInfo()));
    ...
    cv::Mat colour_mat, depth_mat, aligned_depth_to_colour_mat, aligned_depth_vis_mat, ir_mat;
    PsImu imu_data = {0};
    ...
    while (ros::ok()) {
        ...
        PsGetFrame(this->device_index_, PsIRFrame, &ir_frame);
        ...
        if (colour_frame.pFrameData == NULL || depth_frame.pFrameData == NULL ||
            ir_frame.pFrameData == NULL ||aligned_depth_to_colour_frame.pFrameData == NULL) {
            ROS_WARN("Could not get frames: ir=%s, rgb=%s, depth=%s, aligned_depth_to_rgb=%s",
                     ir_frame.pFrameData == NULL ? "failed" : "passed",
        ...
        ir_mat = cv::Mat(ir_frame.height, ir_frame.width, CV_16UC1, ir_frame.pFrameData);
        ...
        // ir_ci->height = ir_frame.height;
        // ir_ci->width = ir_frame.width;
        // ir_ci->header.stamp = now;
