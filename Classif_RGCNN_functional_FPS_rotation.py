...
class cls_model(nn.Module):
    ...
    def forward(self, x,x2,k,batch_size,num_points,sccs):
        self.regularizers = []
        with torch.no_grad():
...
def train(model, optimizer, loader,k,num_points, regularization):
    ...
    for i, data in enumerate(loader):
        ...
        logits, regularizers  = model(x=x.to(device),x2=x2.to(device),k=k,batch_size=data.batch.unique().shape[0],num_points=num_points,sccs=sccs_batch)
