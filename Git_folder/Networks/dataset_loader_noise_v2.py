...
class PcdDataset(Dataset):
        ...
        for category in self.classes.keys():
            ...
            for file in os.listdir(new_dir):
                if file.endswith('.pcd'):
                    ...
                    self.target_pcds.append(sample)
                    break
    ...
    def __preproc__(self, file, idx):
        ...
        path_target=self.target_pcds[self.classes[self.files[idx]['category']]]['pcd_path']



        with open(path_target, 'r') as f_target:
            target=o3d.io.read_point_cloud(f_target.name.strip())
        #print(file)
        ...
        aabb_1 = target.get_oriented_bounding_box()
        aabb_1.color = (1, 0, 0)
        ...
        centroid_1= o3d.geometry.PointCloud.get_center(target)
        centroid_2= o3d.geometry.PointCloud.get_center(pcd)


        #o3d.visualization.draw_geometries([target,pcd,aabb_1,aabb_2])


        pcd=pcd.rotate(aabb_2.R.T)
        target=target.rotate(aabb_1.R.T)


        target.translate(-centroid_1)
        ...
        dists_1 = target.compute_point_cloud_distance(pcd)
    
        pcd=rotate_pcd(pcd,np.pi,2)

        dists_2=target.compute_point_cloud_distance(pcd)

        ceva=np.subtract(dists_1,dists_2)
        ceva2=ceva<0
        count = np.count_nonzero(ceva2)

        if(count>len(pcd.points)/2):
            pcd=rotate_pcd(pcd,-np.pi,2)
        ...
        # cloud_2.translate(centroid_1)
        # cloud_2=cloud_2.rotate(aabb_1.R)

        aabb_2 = pcd.get_oriented_bounding_box()
        aabb_2.color = (0, 0, 1)

        #o3d.visualization.draw_geometries([target,cloud_2,aabb_1,aabb_2])
        o3d.visualization.draw_geometries([target,pcd,aabb_1,aabb_2])
...
if __name__ == '__main__':
    ...
    num_points = 512
    root = Path("/media/rambo/ssd2/Alex_data/RGCNN/PCD_DATA/Normals/Test/Modelnet_40_1cat/")

    root_noise_1 = Path("/media/rambo/ssd2/Alex_data/RGCNN/PCD_DATA/Normals/Test/Modelnet_40_1cat_"+str(num_points)+"_test"+"/")
