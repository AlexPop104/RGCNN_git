...
def test(model, loader,num_points,criterion,device):
    ...
    for data in loader:
        ...
        for j in range(10000):
            ...
            start = time.time()
            logits, regularizers = model(x=x)
            stop = time.time()

            f = open("log_RGCNN_bb_net_"+str(num_points)+".txt", "a")
    ...
    if(selection==1):
        ...
        for i in range(len(array_points)):
            ...
            if (train_select!=1)and (data_select==1):
                path_saved_model="/media/cuda/ssd/Alex/RGCNN/Trained+models/Noise_train_512/Rotation/RGCNN_bb_Rotation512_250.pt"
            elif (data_select==2):
                path_saved_model="/media/cuda/ssd/Alex/RGCNN/Trained+models/Noise_train_512/Position/RGCNN_bb_Pos_noise_512_250.pt"
            elif (data_select==3):
                path_saved_model="/media/cuda/ssd/Alex/RGCNN/Trained+models/Noise_train_512/Occlusion/RGCNN_bb_Occlusion512_99.pt"
            
            else:
                path_saved_model="/media/cuda/ssd/Alex/RGCNN/Trained+models/"+str(num_points)+"/RGCNN_"+str(num_points)+"_bb.pt"
