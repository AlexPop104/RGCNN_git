...
class PcdDataset(Dataset):
    ...
    def __preproc__(self, file, lbl, idx):
        ...
        pointcloud = torch_geometric.data.Data(normal=normals, pos=points, y=labels)

        if self.transforms:
            pointcloud = self.transforms(pointcloud)

        pcd.points=o3d.utility.Vector3dVector(pointcloud.pos)
...
if __name__ == '__main__':
    ...
    num_points = 400
    root = Path("/home/alex/Alex_documents/RGCNN_git/data/Dataset_Rambo_processed_2/")

    root_noise_1 = Path("/home/alex/Alex_documents/RGCNN_git/data/dataset3/")

    if(choice==1):
    ####Processing the datasets
         #process_dataset(root=root, save_path=root_noise_1,  num_points=num_points,transform=test_transform)
         process_dataset(root=root, save_path=None,  num_points=num_points,transform=test_transform)
